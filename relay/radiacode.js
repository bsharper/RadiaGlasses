...
class RadiaCodeBluetoothTransport extends RadiaCodeTransport {
    ...
    /**
     * Reconnect to a previously-paired BluetoothDevice without showing the picker.
     * Browsers allow calling device.gatt.connect() without a user gesture after
     * the device has been paired in the same origin.
     * @param {BluetoothDevice} bluetoothDevice - the saved BluetoothDevice reference
     * @returns {RadiaCodeBluetoothTransport}
     */
    static async reconnect(bluetoothDevice) {
        const transport = new RadiaCodeBluetoothTransport();
        transport.device = bluetoothDevice;
        transport.device.addEventListener('gattserverdisconnected', transport.onDisconnected.bind(transport));
        transport.server = await bluetoothDevice.gatt.connect();
        transport.service = await transport.server.getPrimaryService(RADIACODE_SERVICE_UUID);
        transport.writeCharacteristic = await transport.service.getCharacteristic(WRITE_CHARACTERISTIC_UUID);
        transport.notifyCharacteristic = await transport.service.getCharacteristic(NOTIFY_CHARACTERISTIC_UUID);
        await transport.notifyCharacteristic.startNotifications();
        transport.notifyCharacteristic.addEventListener('characteristicvaluechanged', transport.handleNotification.bind(transport));
...
class RadiaCodeUSBTransport extends RadiaCodeTransport {
    ...
    /**
     * Reconnect to a previously-claimed USBDevice without showing the picker.
     * The browser allows calling device.open() on an already-permitted device
     * without a user gesture.
     * @param {USBDevice} usbDevice - the saved USBDevice reference
     * @returns {RadiaCodeUSBTransport}
     */
    static async reconnect(usbDevice) {
        const transport = new RadiaCodeUSBTransport();
        transport.device = usbDevice;
        await usbDevice.open();
        if (usbDevice.configuration === null) {
            await usbDevice.selectConfiguration(1);
        }
        transport.interface = usbDevice.configuration.interfaces[0];
        await usbDevice.claimInterface(transport.interface.interfaceNumber);
        transport.endpointOut = 1;
        transport.endpointIn = 1;
        transport.isConnected = true;
        return transport;
    }
